	...
	<script type="text/javascript">
		...
		function initTable(){
			$("#mainTable").datagrid({
				...
				columns:[[
					...
					{field:'nam',width:80,align:'center',title:'查看详情',
						formatter:function(value,row,index){
							if(row.taskStatus == 2 || row.taskStatus == 3 || row.taskStatus == '' || row.taskStatus == -1 ){
				...
				url:'${pageContext.request.contextPath}/knowFeeling/doPageListJson.action',
				title:'统计任务列表',
				fitColumns:true,
				//填满区域
				fit:true,
		...
		/*查看详情*/
		function seeInfo(id){
			goToPage("${pageContext.request.contextPath}/knowFeeling/goSee.action?taskId="+id);
			...
			if (row.length!=0) {
				$.messager.confirm("系统提示", "确定要删除选中任务？", function(r) {
					if (r) {
						...
						$.post('${pageContext.request.contextPath}/knowFeeling/delReport.action', {ids : idsString},
							function(result) {
								if (result.errorMsg) {
		...
		function query(){
			...
			if($("#queryForm").form('validate')){
				...
				$("#mainTable").datagrid('load',{
					...
					qCityIds:cityIdsString,
					qBeginDate:$("#beginDate").datetimebox('getValue'),
					qEndDate:$("#endDate").datetimebox('getValue'),
				});
			}
		}
		...
		function add(){
			goToPage('${pageContext.request.contextPath}/knowFeeling/goAdd.action');
		}
	</script>
  </head>
	    	...
	    	<div style="width:100%;height:10%;" data-options="region:'north',title:'筛选条件设置',onCollapse:function(){
	    			...
	    			<form id="queryForm" class="esyui-form" method="post" style="width:100%;height: 100%;margin:0;" >
						...
						<div class="inputDivShow">区域
							<select id="areaTree" name="cityIds"  class="easyui-combotree" data-options="panelWidth:200,editable:false,onlyLeafCheck:true,multiple:true,lines:true,checkbox:false,url:'${pageContext.request.contextPath}/terminalGroup/terminalGroupTree.action'"  >
							</select>
						</div>
