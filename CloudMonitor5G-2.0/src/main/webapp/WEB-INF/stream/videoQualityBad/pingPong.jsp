...
<html>
  <head>
	...
	function doAnalysis(){
		...
		// 以ajax方式，发送到服务器，完成操作
		$.post("${pageContext.request.contextPath}/streamQuality/doPingPongAnalysis.action",{roadId:pingPongRow.id},
			function(result){
				if (result.errorMsg) {
					...
					$("#pingPongAdviceTable").datagrid('loadData',result.pingPongAdvice);
					$("#pingPongKeyTable").datagrid('loadData',result.pingPongKey);
					$("#pingPongCutEventTable").datagrid('loadData',result.pingPongCutEvent);
				}
			}
		,"json");
		$.post("${pageContext.request.contextPath}/streamQuality/checkDrawCellFileName.action",{roadId:pingPongRow.id},
			function(result){
  ...
  <body id="mainLayout" class="easyui-layout" style="width: 100%;height: 100%;margin: 0 auto;padding: 0 auto;list-style:none;overflow:hidden;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=52" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
	    				...
	    				<tbody>
	    					<c:forEach items="${pingPongs}" var="pingPong">
		    					<tr>
		    						...
		    						<td>${pingPong.timeInterval}</td>
		    						<td>${pingPong.m_stRoadName}</td>
	    							<td>${pingPong.distance}</td>
		    						<td>${pingPong.vmosAvg}</td>
		    						<td>${pingPong.critialCause}</td>
		    						<td>${pingPong.videoresolutionAvg}</td>
		    						<td>${pingPong.testLogItem.fileName}</td>
		    						<td>${pingPong.testLogItem.recSeqNo}</td>
		    						<td>${pingPong.beginLatitude}</td>
		    						<td>${pingPong.courseLatitude}</td>
		    						<td>${pingPong.endLatitude}</td>
		    						<td>${pingPong.beginLongitude}</td>
		    						<td>${pingPong.courseLongitude}</td>
		    						<td>${pingPong.endLongitude}</td>
		    					</tr>
	    					</c:forEach>
			    ...
			    <div data-options="region:'center',border:false" style="padding:2px 0px 0px 0px;">
				    <div class="easyui-layout" style="width:100%;height: 100%;">
				    	<div data-options="region:'north',border:false" style="height:40%;">
		  					<table id="pingPongKeyTable" class="easyui-datagrid" data-options="title:'问题路段指标',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
			    				<thead>
			    					<tr>
		  					...
		  					<table id="pingPongAdviceTable" class="easyui-datagrid" data-options="title:'优化建议:调整切换参数',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
			    				<thead>
			    					...
			    					<tr>
			    						...
			    						<th data-options="field:'cellPci',width:60,align:'center'">PCI</th>
			    						<th data-options="field:'cellEarfcn',width:90,align:'center'">EARFCN</th>
			    						<th data-options="field:'cellRsrp',width:60,align:'center',formatter:numToFixed2Formatter">RSRP</th>
			    						<th data-options="field:'cellToProblemDotDistance',width:90,align:'center',formatter:numToFixed2Formatter">和问题采样点<br>距离(m)</th>
			    						<th data-options="field:'ncellName',width:90,align:'center',formatter:showTooltip">小区名</th>
			    						<th data-options="field:'ncellPci',width:60,align:'center'">PCI</th>
			    						<th data-options="field:'ncellEarfcn',width:90,align:'center'">EARFCN</th>
			    						<th data-options="field:'ncellRsrp',width:60,align:'center',formatter:numToFixed2Formatter">RSRP</th>
			    						<th data-options="field:'ncellToProblemDotDistance',width:90,align:'center',formatter:numToFixed2Formatter">和问题采样点<br>距离(m)</th>
