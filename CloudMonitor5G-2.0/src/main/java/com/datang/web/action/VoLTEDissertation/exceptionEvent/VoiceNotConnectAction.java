...
/**
 * volte异常事件---volte语音未接通
 * 
 * @author yinzhipeng
 * @date:2016年4月13日 上午10:25:53
 * @version
 */
@SuppressWarnings("all")
@Controller
@Scope("prototype")
public class VoiceNotConnectAction extends PageAction {
	...
	public String roadNameCallBack() {
		if (null != rowIDs && rowIDs.length != 0) {
			for (int i = 0; i < rowIDs.length; i++) {
				volteExceptionEventService.addQBRRoadName(rowRoadNames[i],
						rowIDs[i]);
			}
		}
		return null;
	...
	public String doWholeAnalysis() {
		...
		if (null != attribute) {
			if (attribute instanceof String) {
				...
				List<VolteExceptionEventNotConnect> volteExceptionEventNotConnectByLogIds = volteExceptionEventService
						.getVolteExceptionEventNotConnectByLogIds(ids);
				ActionContext
						.getContext()
						.getValueStack()
						.push(volteExceptionEventService
								.doNotConnectWholeAnalysis(
										volteExceptionEventNotConnectByLogIds,
										queryTestLogItems));
			}
			ServletActionContext.getRequest().getSession()
					.setAttribute("testLogItemIds", attribute);
