...
<html>
  <head>
    <title>VoLte异常事件----VoLte视频未接通事件问题分析</title>
	...
	function doAnalysis(){
		...
		if(lastVolteEEid!=volteEEid){
			...
			$("#notConnectSignallingTable").datagrid({
				...
				url:'${pageContext.request.contextPath}/videoNotConnect/doPageListJson.action?isOpposite=false&exceptionEventId='+volteEEid,
			...
			$("#notConnectOppositeSignallingTable").datagrid({
				// 表头
				columns:signallingTableColumns,
				url:'${pageContext.request.contextPath}/videoNotConnect/doPageListJson.action?isOpposite=true&exceptionEventId='+volteEEid,
	...
	<c:if test="${null == VideoNotConnects || empty VideoNotConnects}">
		isEESortable = false;
	</c:if>
	$(function(){
		<c:if test="${null == VideoNotConnects || empty VideoNotConnects}">
  ...
  <body class="easyui-layout" id="mainLayout" style="width: 100%;height: 100%;margin: 0 auto;padding: 0 auto;list-style:none;overflow:hidden;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" style="width: 100%;height: 100%;" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=44" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
    	<div data-options="region:'east',border:false" style="width:55%;padding:0px 0px 0px 2px;">
    		<div class="easyui-layout" style="width:100%;height: 100%;">
		    	...
		    	<div data-options="region:'north',border:false" style="height:24%;">
  					<table id="notConnectEventTable" class="easyui-datagrid" data-options="title:'视频未接通问题列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,rownumbers:true,singleSelect:true,tools:'#tt3',onLoadSuccess:mainTableLoadSuccess,remoteSort:false,multiSort:true"  >
	    				<thead>
	    					<tr>
	    				...
	    				<tbody>
	    					<c:forEach items="${VideoNotConnects}" var="notConnect">
		    					<tr>
		    						<td>${notConnect.id}</td>
	    					...
	    					<c:if test="${null == VideoNotConnects}">
	    						<tr>
		    						<td></td>
		    						<td>未选择测试日志</td>
		    						<td></td>
