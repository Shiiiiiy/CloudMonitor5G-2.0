 ...
 * 
 * @explain
 * @name VideoDropCallAction
 * @author shenyanwei
 * @date 2017年5月11日上午10:07:48
 */
@SuppressWarnings("all")
@Controller
@Scope("prototype")
public class VideoDropCallAction extends PageAction {
	...
	public String doWholeAnalysis() {
		...
		if (null != attribute) {
			if (attribute instanceof String) {
				...
				List<VideoExceptionEventDropCall> videoExceptionEventDropCallByLogIds = volteExceptionEventService
						.getVideoExceptionEventDropCallByLogIds(ids);
				ActionContext
						.getContext()
						.getValueStack()
						.push(volteExceptionEventService
								.doVideoDropCallWholeAnalysis(
										videoExceptionEventDropCallByLogIds,
										queryTestLogItems));
			}
			ServletActionContext.getRequest().getSession()
					.setAttribute("testLogItemIds", attribute);
	...
	public String haveTestLogMeasure() {
		...
		ActionContext
				...
				.getValueStack()
				.push(volteExceptionEventService
						.queryTestLogMeasuresByTimePage(begainDate, endDate,
								rows, page));
		return ReturnType.JSON;
	}
