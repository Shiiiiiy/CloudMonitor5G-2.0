...
<html>
  <head>
	...
	function doAnalysis(){
		...
		// 以ajax方式，发送到服务器，完成操作
		$.post("${pageContext.request.contextPath}/videoQuality/doNeighbourPlotAnalysis.action",{roadId:nbCellRow.id},
			function(result){
				if (result.errorMsg) {
					$.messager.alert("系统提示", result.errorMsg,'error');
				} else {
					$("#nbPlotAdviceTable").datagrid('loadData',result.neighbourPlotAddNeighbourPlot);
				}
			}
		,"json");
		$.post("${pageContext.request.contextPath}/videoQuality/checkDrawCellFileName.action",{roadId:nbCellRow.id},
			function(result){
  ...
  <body id="mainLayout" class="easyui-layout" style="width: 100%;height: 100%;margin: 0 auto;padding: 0 auto;list-style:none;overflow:hidden;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=33" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
	    				...
	    				<tbody>
	    					<c:forEach items="${neighbourPlots}" var="neighbourPlot">
		    					<tr>
		    						...
		    						<td>${neighbourPlot.testLogItem.callType}</td>
		    						<td>${neighbourPlot.timeValue}</td>
		    						<td>${neighbourPlot.vmos}</td>
	    							<td>${neighbourPlot.keyParameterCause}</td>
		    						<td>${neighbourPlot.cellName}</td>
		    						<td>${neighbourPlot.optimization}</td>
		    						<td>${neighbourPlot.testLogItem.fileName}</td>
		    						<td>${neighbourPlot.testLogItem.recSeqNo}</td>
		    						<td>${neighbourPlot.latitude}</td>
		    						<td>${neighbourPlot.latitude}</td>
		    						<td>${neighbourPlot.latitude}</td>
		    						<td>${neighbourPlot.longitude}</td>
		    						<td>${neighbourPlot.longitude}</td>
		    						<td>${neighbourPlot.longitude}</td>
  					...
  					<table id="nbPlotAdviceTable" class="easyui-datagrid" data-options="title:'优化建议:添加邻区',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
	    				<thead>
	    					<tr>
	    						...
	    						<th data-options="field:'toProblemDotDistance',width:90,align:'center',formatter:numToFixed2Formatter">和问题采样点距离(m)</th>
	    					</tr>
	    				</thead>
    					<tbody>
    						<tr>
