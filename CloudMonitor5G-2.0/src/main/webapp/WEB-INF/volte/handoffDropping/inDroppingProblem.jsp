...
<html>
  <head>
    <title>VoLte异常事件----系统内部切换失败问题分析</title>
	...
	var mainTableId = 'droppingIntsTable';
	function getRoadNameCallBackUrl(){
		return "<%=basePath%>droppingInt/roadNameCallBack.action";
	}
	//存储日志id,按","分隔
				...
				function(result){
					if(result.errorMsg){
						...
						if(!signllingDiv){
							...
							$('#signllingDiv'+rowData.id).dialog({
							    ...
							    modal: false,
							    onClose:function(){
							    	$('#signllingDiv'+rowData.id).dialog('destroy',true);
							    }
							});
						}
					}
	...
	var signallingTableColumns=[[
		...
		var lastVolteEERid;//上一次失败事件id
	/* 开始分析,分析按钮 */
	function doAnalysis(){
		var droppingInts = $("#droppingIntsTable").datagrid('getSelected');//获取表格中用户选中数据
		if(!droppingInts){// 操作前至少选中一条
			$.messager.alert("提示","请勾选某个失败事件!",'warning');
			return ;
		}
		testLogItemIds = droppingInts.recSeqNo;
		volteHOFid = droppingInts.id;
		if(!volteHOFid){
			$.messager.alert("提示","请勾选某个失败事件!",'warning');
			return ;
		}
		...
		$.post("${pageContext.request.contextPath}/droppingInt/haveTestLogMeasure.action",{droppingId:volteHOFid},
				function(result){
		$("#callAndOtherCall").datagrid('loadData',result);
								}
		,"json");
		var roadName = droppingInts.m_stRoadName;
		//本次和上一次的失败事件id不同
		if(lastVolteEERid!=volteHOFid){
			$("#intsSignallingTable").datagrid('loadData',[]);
			$("#intsSignallingTable").datagrid({
				// 表头
				columns:signallingTableColumns,
				url:'${pageContext.request.contextPath}/droppingInt/doPageListJson.action?isOpposite=false&droppingId='+volteHOFid,
				title:roadName+'系统内部切换失败信令列表',
				fitColumns:true,
				//填满区域
				fit:true,
				//奇偶变色
			...
			$('#intsSignallingTable').datagrid('getPanel').addClass("lines-no");
		}
		lastVolteEERid = volteHOFid;
		$.post("${pageContext.request.contextPath}/voiceQBR/checkDrawCellFileName.action",{roadId:droppingInts.id},
			function(result){
	...
	<c:if test="${null == droppingInts || empty droppingInts}">
		isRoadSortable = false;
	</c:if>
	$(function(){
		<c:if test="${null == droppingInts || empty droppingInts}">
			$("#droppingIntsTable").datagrid('mergeCells',{
				index: 0,
				field: 'boxid',
				colspan:6
			});
		...
		$("#intsSignallingTable").datagrid('mergeCells',{
			index: 0,
			field: 'time',
			colspan:6
		});
    	...
    	<div data-options="region:'east',border:false" style="width:55%;padding:0px 0px 0px 2px;">
    		<div class="easyui-layout" style="width:100%;height: 100%;">
		    	...
		    	<div data-options="region:'north',border:false" style="height:45%;">
  					<table id="droppingIntsTable" class="easyui-datagrid" data-options="title:'系统内部切换失败问题列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,rownumbers:true,singleSelect:true,tools:'#tt3',onLoadSuccess:mainTableLoadSuccess,remoteSort:false,multiSort:true"  >
	    				<thead>
	    					<tr>
	    				...
	    				<tbody>
	    					<c:forEach items="${droppingInts}" var="droppingInt">
		    					<tr>
		    						<td>${droppingInt.id}</td>
		    						<td>${droppingInt.testLogItem.boxId}</td>
		    						<td>${droppingInt.m_stRoadName}</td>
		    						<td>${droppingInt.failDateTime}</td>
		    						<td>${droppingInt.cellName}</td>
		    						<td>${droppingInt.failName}</td>
		    						<td>${droppingInt.testLogItem.fileName}</td>
		    						<td>${droppingInt.testLogItem.recSeqNo}</td>
		    						<td>${droppingInt.beginLatitude}</td>
		    						<td>${droppingInt.courseLatitude}</td>
		    						<td>${droppingInt.endLatitude}</td>
		    						<td>${droppingInt.beginLongitude}</td>
		    						<td>${droppingInt.courseLongitude}</td>
		    						<td>${droppingInt.endLongitude}</td>
		    					</tr>
	    					</c:forEach>
	    					<c:if test="${null == droppingInts}">
	    						<tr>
		    						<td></td>
		    						<td>未选择测试日志</td>
		    						<td></td>
				...
				<div data-options="region:'center',border:false" style="padding:2px 0px 0px 0px;">
					<table id="intsSignallingTable" class="easyui-datagrid" data-options="title:'信令列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
	    				<thead>
	    					<tr>
