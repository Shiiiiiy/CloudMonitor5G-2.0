  ...
  <head>
	...
	<script type="text/javascript">
		...
		var testLogInfoColumns=[[ 
			...
			{field:'channelNo',width:60,align:'center',title:'通道'},
			{field:'channelType',width:60,align:'center',title:'通道类型'},
			{field:'mosValue',width:60,align:'center',title:'MOS值'},
			{field:'mosTime',width:90,align:'center',title:'MOS时间'}
		...
		function initTable(){
			$("#testLogInfoTable").datagrid({
				...
				url:'${pageContext.request.contextPath}/mosValue/doPageListJson.action',
				title:'MOS监控列表',
				fitColumns:true,
				//填满区域
				fit:true,
		...
		function query(){
			if($("#queryForm").form('validate')){
				...
				$("#testLogInfoTable").datagrid('load',{
					...
					boxIds:boxIdsString,
					minMosValue:$("#minMosValue").textbox('getValue'),
					maxMosValue:$("#maxMosValue").textbox('getValue'),
    	...
    	<div id="ly" class="easyui-layout"  style="width:100%;height: 100%;padding:4px;top:4px;left:4px;position: absolute;">
	    	...
	    	<div data-options="region:'north',title:'筛选条件设置',onCollapse:function(){
    			...
    			<form id="queryForm" class="esyui-form" method="post" style="width:100%;height: 100%;margin:0;" >
			    	...
			    	<div class="inputDivShow">MOS起始值
			    		<input id="minMosValue" name="minMosValue"  class="easyui-numberbox" data-options="validType:'length[1,30]'"/>
			    	</div>
			    	<div class="inputDivShow">MOS最大值
			    		<input id="maxMosValue" name="maxMosValue"  class="easyui-numberbox" data-options="validType:'length[1,30]'"/>
			    	</div>
