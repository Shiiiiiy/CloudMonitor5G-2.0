...
(function(){
	...
	Main.prototype = {
		...
		onSelect : function(list) {
			fShowMessage("文件数量: " + list.length);
		},
		onFileCountExceed : function(selected, limit) {
			fShowMessage("选中文件数量:" + selected + ",超出最大上传个数:" + limit, true);
		},
		onMaxSizeExceed : function(size, limited, name) {
			fShowMessage("文件:" + name + "的大小:" + (size/1024/1024).toFixed(2) +"MB超出单个文件大小的最大限制:" + (limited/1024/1024).toFixed(2)+"MB", true);
		},
		onExtNameMismatch: function(name, filters) {
			fShowMessage("文件类型错误:"+name+",允许的文件类型为: [" + filters.toString() + "]",true);
		},
		onCancel : function(info) {
			fShowMessage("取消: " + info.name);
		},
		onStop : function() {
			fShowMessage("停止!");
		},
		onCancelAll : function(numbers) {
			fShowMessage(numbers + "个文件被取消 ");
		},
		onComplete : function(info) {
			fShowMessage("文件:" + info.name + ",大小:" + info.size + "Byte,上传完成[ok]");
		},
		onQueueComplete : function(info) {
			fShowMessage("所有文件上传完成	====>	[OK]");
		},
		onUploadError : function(status, msg) {
			fShowMessage("上传错误.响应状态:" + status + ", 错误提示: " + msg.message, true);
		},
		disable : function() {
			...
			xhr.onreadystatechange = function() {
				...
				try {
					...
					if (token) {
						...
						fShowMessage("文件:"+self.uploadInfo[index].file.config.name+"系统已存在,自动取消上传!",true);
