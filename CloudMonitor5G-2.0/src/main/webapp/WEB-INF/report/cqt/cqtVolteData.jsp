...
<html>
  <head>
	...
	$(function(){
		$('#tt').tabs({
			tools:[{
				...
				iconCls:'icon-excel',
				handler:function(){
					goToPage("${pageContext.request.contextPath}/cqtReport/downloadOneSheetLteDataTotalExcel.action?sheetName="+sheetName+"");
				}
			},{
				iconCls:'icon-batchexcel',
				text:'批量导出',
				handler:function(){
					goToPage('${pageContext.request.contextPath}/cqtReport/downloadLteDataTotalExcel.action');
				}
			}]
		});
		initTable();
	...
	//初始化数据
	function initTable(){
		$.post("${pageContext.request.contextPath}/cqtReport/quaryDataKpi.action",{},
			function(result){
				if (result.errorMsg) {
					...
					if(collectTypes){
						for(var i=1;i<10;i++){
							...
							if(s==-1){
								...
								$('#Table8').datagrid('hideColumn',valu);
								$('#Table9').datagrid('hideColumn',valu);
							}
						}
				}
					
					$("#Table1").datagrid('loadData',result.info.indexKpi);
					$("#Table2").datagrid('loadData',result.info.coverKpi);
					$("#Table3").datagrid('loadData',result.info.disturbKpi);
					$("#Table4").datagrid('loadData',result.info.dispatcherKpi);
					$("#Table5").datagrid('loadData',result.info.moveKpi);
					$("#Table6").datagrid('loadData',result.info.insertKpi);
					$("#Table7").datagrid('loadData',result.info.ftpSectionKpi);
					$("#Table8").datagrid('loadData',result.info.networkWerkcoverKpi);
					$("#Table9").datagrid('loadData',result.info.stopKpi);
  					...
  					<table id="Table1" class="easyui-datagrid" data-options="scrollbarSize:0,striped:true,fitColumns:false,singleSelect:true"  >
	    				<thead>
	    					...
	    					<tr>
	    						...
	    						<th data-options="field:'ftptimedltdl',width:80,align:'center'">TDL驻网时长(s)(业务态)</th>
	    						<th data-options="field:'ftptimedltds',width:80,align:'center'">TDS驻网时长(s)(业务态)</th>
	    						<th data-options="field:'ftptimedlgsm',width:80,align:'center'">GSM驻网时长(s)(业务态)</th>
	    						<th data-options="field:'ftptimeratiodltdl',width:80,align:'center'">TDL驻网时长占比(业务态)</th>
	    						<th data-options="field:'ftptimeratiodltds',width:80,align:'center'">TDS驻网时长占比(业务态)</th>
	    						<th data-options="field:'ftptimeratiodlgsm',width:80,align:'center'">GSM驻网时长占比(业务态)</th>
