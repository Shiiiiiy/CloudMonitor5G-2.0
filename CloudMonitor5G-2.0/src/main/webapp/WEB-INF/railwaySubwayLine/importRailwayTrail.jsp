...
<!--     <script type="text/javascript" src="http://api.map.baidu.com/api?v=2.0&ak=liKpDfLP41rNnZmM1D33WljN"></script> -->
	...
	function saveImport(){
		$("#saveForm").form('submit',{
			url:"${pageContext.request.contextPath}/railwayLine/importRailwayTrail",
			success:function(result){
				var resultData =$.parseJSON(result);
				if (resultData.errorMsg) {
					$.messager.alert("系统提示", resultData.errorMsg,"error");
				} else {
					if(null!=resultData.xmlNum){
						parent.parent.messagerAlert("导入成功", "<strong>成功导入"+resultData.xmlNum+"条xml","info");
					}
					$("#saveForm").form('reset');
				}
  ...
  <body style="width:100%;height: 100%;margin: 0 auto;list-style:none;overflow:hidden;">
    	<form method="post" style="margin-bottom: 5px;" id="saveForm" enctype="multipart/form-data">
	    	<div class="inputDivShow">文件:
	    		<input class="easyui-filebox" name="importFile" validType="fileType['csv']" data-options="buttonText:'浏览',required:true,invalidMessage:'文件类型必须为.csv'" />
	    	</div>
