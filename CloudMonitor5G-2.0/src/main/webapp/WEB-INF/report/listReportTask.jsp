	...
	<script type="text/javascript">
		...
		function initTable(){
			$("#mainTable").datagrid({
				...
				url:'${pageContext.request.contextPath}/report/doPageListJson.action',
				title:'统计任务列表',
				fitColumns:true,
				//填满区域
				fit:true,
		...
		/*查看详情*/
		function seeInfo(id){
			goToPage("${pageContext.request.contextPath}/report/seeInfo.action?idLong="+id+"");
		}
		/* 跳转到添加任务页面 */
		function add(){
			goToPage('${pageContext.request.contextPath}/report/goAdd.action');
		}
		/* 删除任务*/
		function delReport(){
			...
			if (row.length!=0) {
				$.messager.confirm("系统提示", "确定要删除选中任务？", function(r) {
					if (r) {
						...
						$.post('${pageContext.request.contextPath}/report/delReport.action', {ids : idsString}, 
							function(result) {
								if (result.errorMsg) {
