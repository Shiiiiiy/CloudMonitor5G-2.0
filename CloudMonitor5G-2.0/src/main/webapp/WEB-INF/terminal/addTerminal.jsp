...
<html>
	...
	<script type="text/javascript">
		...
		function testTargetSelect(selectOption){
			...
			switch(selectOption.value){
				...
				case '2':
					...
					break;
				case '3':
					boxidValue = '5'+subBoxidValue;
					break;
				default:
					boxidValue = subBoxidValue;
			}
		...
		$(function(){
		    ...
		    $("input",$("#te_boxid").next("span")).keyup(function(){
				...
				$("#te_boxid").textbox('setValue',boxidValue); 
				$("#name").textbox('setValue',boxidValue); 
		    });
		})
	</script>
  </head>
  ...
  <body style="width:100%;height: 100%;margin: 0 auto;list-style:none;overflow:hidden;">
    	<form method="post" style="margin-bottom: 5px;" action="${pageContext.request.contextPath}/terminal/addTeBaseInfo.action?cityId=${id}" id="addTerminalForm" >
	    	<div class="inputDivShow">终端名称
	    		<input id="name" name="name"  class="easyui-textbox" data-options="required:true,validType:'length[1,24]'" />
	    	</div>
	    	...
	    	<div class="inputDivShow">测试计划版本
	    		<input name="testPlanVersion"  class="easyui-textbox" data-options="validType:'length[1,24]',readonly:true" />
	    	</div>
	    	...
	    	<table width="100%" >
		    	<tr >
			    	<td width="50%" style="padding-right:10px;" align="right"><a class="easyui-linkbutton" style="width:80px" onclick="addTerminal();" data-options="iconCls:'icon-save'">保存</a></td>
   		...
   		<div id="testModuleInfoDlg" class="easyui-dialog" style="width:600px;height:370px;padding:5px 0px;" data-options="onMove:onDrag,closed:true,modal:true,buttons:'#testModuleInfoDlg-buttons'">
    		<form id="testModuleInfoForm" method="post" action="${pageContext.request.contextPath}/testModule/saveOrUpdate.action">
				...
				<div class="inputDivShow">sim卡号
					<input id="simCard" class="easyui-textbox" name="simCard">
				</div>
				...
				<div class="inputDivShow">波特率
					<input class="easyui-numberbox"  name="baudRate">
				</div>
