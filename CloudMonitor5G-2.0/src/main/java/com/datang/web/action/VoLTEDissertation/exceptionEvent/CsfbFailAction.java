 ...
 * 
 * @author yinzhipeng
 * @date:2016年4月13日 上午10:49:17
 * @version
 */
@SuppressWarnings("all")
@Controller
@Scope("prototype")
public class CsfbFailAction extends PageAction {
	 ...
	 * @return
	 */
	public String goToCsfbFailList() {
	...
	public String doWholeAnalysis() {
		...
		if (null != attribute) {
			if (attribute instanceof String) {
				...
				List<VolteExceptionEventCsfbFail> volteExceptionEventCsfbFailByLogIds = volteExceptionEventService
						.getVolteExceptionEventCsfbFailByLogIds(ids);
				ActionContext
						.getContext()
						.getValueStack()
						.push(volteExceptionEventService
								.doCsfbFailWholeAnalysis(
										volteExceptionEventCsfbFailByLogIds,
										queryTestLogItems));
			}
			ServletActionContext.getRequest().getSession()
					.setAttribute("testLogItemIds", attribute);
		...
		if (null != attribute) {
			if (attribute instanceof String) {
				...
				List<VolteExceptionEventCsfbFail> volteExceptionEventCsfbFailByLogIds = volteExceptionEventService
						.getVolteExceptionEventCsfbFailByLogIds(ids);
				ActionContext.getContext().getValueStack()
						.set("csfbFails", volteExceptionEventCsfbFailByLogIds);
				ActionContext.getContext().getValueStack()
						.set("testLogItemIds", testLogItemIds);
			}
			ServletActionContext.getRequest().getSession()
