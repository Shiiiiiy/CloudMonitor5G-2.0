...
<html>
  <head>
	...
	/* 开始分析,分析按钮 */
	function doAnalysis(){
		var otherRow = $("#otherTable").datagrid('getSelected');//获取表格中用户选中数据
		...
		// 以ajax方式，发送到服务器，完成操作
		 $.post("${pageContext.request.contextPath}/streamQuality/doOtherAnalysis.action",{roadId:otherRow.id},
			function(result){
				if (result.errorMsg) {
					$.messager.alert("系统提示", result.errorMsg,'error');
				}else {
					$("#otherKeyIndexTable").datagrid('loadData',result.otherKey);
				}
			}
		,"json"); 
		$.post("${pageContext.request.contextPath}/streamQuality/checkDrawCellFileName.action",{roadId:otherRow.id},
			function(result){
	...
	$(function(){
		<c:if test="${null == others || empty others}">
			$("#otherTable").datagrid('mergeCells',{
				index: 0,
				field: 'boxId',
				colspan:7
			});
  ...
  <div id="mainLayout" class="easyui-layout" style="width: 100%;height: 100%;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=58" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
    	<div data-options="region:'east',border:false" style="width:50%;padding:0px 0px 0px 2px;">
    		<div class="easyui-layout" style="width:100%;height: 100%;">
		    	...
		    	<div data-options="region:'north',border:false" style="height:50%;">
		    		<table id="otherTable" class="easyui-datagrid" data-options="title:'其他问题列表',scrollbarSize:0,onLoadSuccess:mainTableLoadSuccess,fit:true,striped:true,fitColumns:true,rownumbers:true,singleSelect:true,tools:'#tt3',remoteSort:false,multiSort:true"  >
			    		<thead>
			    			<tr>
	    				...
	    				<tbody>
	    					<c:forEach items="${others}" var="other">
		    					<tr>
		    						...
		    						<td>${other.timeValue}</td>
		    						<td>${other.timeInterval}</td>
		    						<td>${other.m_stRoadName}</td>
	    							<td>${other.distance}</td>
		    						<td>${other.vmosAvg}</td>
		    						<td>${other.critialCause}</td>
		    						<td>${other.videoresolutionAvg}</td>
		 ...
		 <div data-options="region:'center',border:false" >
			<table id="otherKeyIndexTable" class="easyui-datagrid" data-options="title:'关键指标',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
  				<thead>
  					<tr>
