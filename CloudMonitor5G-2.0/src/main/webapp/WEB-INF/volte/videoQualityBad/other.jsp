...
<html>
  <head>
	...
	function doAnalysis(){
		...
		// 以ajax方式，发送到服务器，完成操作
		/* $.post("${pageContext.request.contextPath}/videoQuality/doOtherAnalysis.action",{roadId:otherRow.id},
			function(result){
				if (result.errorMsg) {
		...
		,"json"); */
		$.post("${pageContext.request.contextPath}/videoQuality/checkDrawCellFileName.action",{roadId:otherRow.id},
			function(result){
  ...
  <div id="mainLayout" class="easyui-layout" style="width: 100%;height: 100%;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=39" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
	    				...
	    				<tbody>
	    					<c:forEach items="${others}" var="other">
		    					<tr>
		    						...
		    						<td>${other.testLogItem.callType}</td>
		    						<td>${other.timeValue}</td>
		    						<td>${other.vmos}</td>
	    							<td>${other.keyParameterCause}</td>
