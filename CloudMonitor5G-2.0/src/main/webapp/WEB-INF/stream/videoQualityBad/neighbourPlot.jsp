...
<html>
  <head>
	...
	/* 开始分析,分析按钮 */
	function doAnalysis(){
		var nbCellRow = $("#nbPlotTable").datagrid('getSelected');//获取表格中用户选中数据
		...
		// 以ajax方式，发送到服务器，完成操作
		$.post("${pageContext.request.contextPath}/streamQuality/doNeighbourPlotAnalysis.action",{roadId:nbCellRow.id},
			function(result){
				if (result.errorMsg) {
					$.messager.alert("系统提示", result.errorMsg,'error');
				} else {
					$("#nbPlotAdviceTable").datagrid('loadData',result.neighbourPlotAdvice);
					$("#nbPlotKeyTable").datagrid('loadData',result.neighbourPlotKey);
				}
			}
		,"json");
		$.post("${pageContext.request.contextPath}/streamQuality/checkDrawCellFileName.action",{roadId:nbCellRow.id},
			function(result){
	...
	<c:if test="${null == neighbourPlots || empty neighbourPlots}">
		isRoadSortable = false;
	</c:if>
	$(function(){
		<c:if test="${null == neighbourPlots || empty neighbourPlots}">
			$("#nbPlotTable").datagrid('mergeCells',{
				index: 0,
				field: 'boxId',
				colspan:8
			});
		...
		$("#nbPlotAdviceTable").datagrid('mergeCells',{
			index: 0,
			field: 'cellName',
			colspan:4
		});
  ...
  <body id="mainLayout" class="easyui-layout" style="width: 100%;height: 100%;margin: 0 auto;padding: 0 auto;list-style:none;overflow:hidden;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=53" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
    	<div data-options="region:'east',border:false" style="width:50%;padding:0px 0px 0px 2px;">
    		<div class="easyui-layout" style="width:100%;height: 100%;">
		    	...
		    	<div data-options="region:'north',border:false" style="height:55%;">
	    			<table id="nbPlotTable" class="easyui-datagrid" data-options="title:'邻区问题列表',scrollbarSize:0,fit:true,striped:true,onLoadSuccess:mainTableLoadSuccess,fitColumns:true,rownumbers:true,singleSelect:true,tools:'#tt3',remoteSort:false,multiSort:true"  >
	    				<thead>
	    					<tr>
	    				...
	    				<tbody>
	    					<c:forEach items="${neighbourPlots}" var="neighbourPlot">
		    					<tr>
		    						<td>${neighbourPlot.id}</td>
		    						<td>${neighbourPlot.testLogItem.boxId}</td>
		    						<td>${neighbourPlot.timeValue}</td>
		    						<td>${neighbourPlot.timeInterval}</td>
		    						<td>${neighbourPlot.m_stRoadName}</td>
	    							<td>${neighbourPlot.distance}</td>
		    						<td>${neighbourPlot.vmosAvg}</td>
		    						<td>${neighbourPlot.critialCause}</td>
		    						<td>${neighbourPlot.videoresolutionAvg}</td>
		    						<td>${neighbourPlot.testLogItem.fileName}</td>
		    						<td>${neighbourPlot.testLogItem.recSeqNo}</td>
		    						<td>${neighbourPlot.beginLatitude}</td>
		    						<td>${neighbourPlot.courseLatitude}</td>
		    						<td>${neighbourPlot.endLatitude}</td>
		    						<td>${neighbourPlot.beginLongitude}</td>
		    						<td>${neighbourPlot.courseLongitude}</td>
		    						<td>${neighbourPlot.endLongitude}</td>
		    					</tr>
	    					</c:forEach>
	    					<c:if test="${null == neighbourPlots}">
	    						<tr>
		    						<td></td>
		    						<td>未选择测试日志</td>
		    						<td></td>
		    	...
		    	<div data-options="region:'center',border:false" >
  					<table id="nbPlotKeyTable" class="easyui-datagrid" data-options="title:'关键指标',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
	    				<thead>
	    					<tr>
			    ...
			    <div data-options="region:'south',border:false" style="height:32%;padding:2px 0px 0px 0px;">
  					<table id="nbPlotAdviceTable" class="easyui-datagrid" data-options="title:'优化建议:添加邻区',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
	    				<thead>
		    				<tr>
	    						<th colspan="5" >服务小区</th>
	 									<th colspan="5" >待添加邻区</th>
	    					</tr>
	    					<tr>
	    						...
	    						<th data-options="field:'toProblemDotDistance',width:90,align:'center',formatter:numToFixed2Formatter">和问题采样<br>点距离(m)</th>
	    						<th data-options="field:'ncellName',width:90,align:'center',formatter:showTooltip">小区名</th>
	    						<th data-options="field:'npci',width:60,align:'center'">PCI</th>
	    						<th data-options="field:'nearfcn',width:90,align:'center'">EARFCN</th>
	    						<th data-options="field:'nrsrp',width:60,align:'center',formatter:numToFixed2Formatter">RSRP</th>
	    						<th data-options="field:'ntoProblemDotDistance',width:90,align:'center',formatter:numToFixed2Formatter">和问题采样<br>点距离(m)</th>
				    		</tr>
	    				</thead>
    					<tbody>
    						<tr>
