...
<html>
  <head>
	...
	function doAnalysis(){
		...
		// 以ajax方式，发送到服务器，完成操作
		$.post("${pageContext.request.contextPath}/volteCWBR/doNbCellRoadAnalysis.action",{roadId:nbCellRow.id},
			function(result){
				if (result.errorMsg) {
  ...
  <body id="mainLayout" class="easyui-layout" style="width: 100%;height: 100%;margin: 0 auto;padding: 0 auto;list-style:none;overflow:hidden;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=15" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
	    				...
	    				<tbody>
	    					<c:forEach items="${nbDeficiencyRoads}" var="nbDeficiencyRoad">
		    					<tr>
	    							...
	    							<td>${nbDeficiencyRoad.startTimeValue}</td>
		    						<td>${nbDeficiencyRoad.mosAvg}</td>
		    						<td>${nbDeficiencyRoad.rsrpAvg}</td>
		    						<td>${nbDeficiencyRoad.sinrAvg}</td>
				   			...
				   			<table id="nbCellLTEAddAdviceCellInfoTable" class="easyui-datagrid" data-options="title:'LTE邻区添加建议',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
				   				<thead>
				   					<tr>
				   						...
				   						<th rowspan="2" data-options="field:'nb_rsrpAvg',width:90,align:'center'" >对该连续无线差路<br>段覆盖强度(dBm)</th>
				   						<th rowspan="2" data-options="field:'nb_distanceAvg',width:70,align:'center'" >和连续无线差<br>路段距离(m)</th>
				   					</tr>
				   					<tr>
