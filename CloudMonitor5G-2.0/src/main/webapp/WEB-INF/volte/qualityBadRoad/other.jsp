...
<html>
  <head>
	...
	function doAnalysis(){
		...
		// 以ajax方式，发送到服务器，完成操作
		$.post("${pageContext.request.contextPath}/voiceQBR/doOtherRoadAnalysis.action",{roadId:otherRow.id},
			function(result){
				if (result.errorMsg) {
					...
					//指标列表
					if(otherRoadName){
						$("#otherIndexTable").datagrid('getPanel').panel('setTitle',otherRoadName+"路段"+$("#otherIndexTable").datagrid('options').title);
					}
					$("#otherIndexTable").datagrid('loadData',result.otherRoadIndex);
					...
					$("#otherCellInfoTable").datagrid({
						...
						columns:[[
							...
							/* {field:'sampleRatio',width:80,align:'center',title:'采样点占整个质差<br>路段采样点比例(%)',formatter:numToFixed2Formatter,sortable:true},
							{field:'cellNumRatio',width:80,align:'center',title:'质差区域采样点与<br>总采样点占比(%)',formatter:numToFixed2Formatter,sortable:true}, */
	...
	function showSignallingPanel(){
		...
		var signllingUrl = '';
		if(volteqbrid){
			signllingUrl = '${pageContext.request.contextPath}/voiceQBR/doPageListJson.action?roadId='+volteqbrid;
		}
  ...
  <div id="mainLayout" class="easyui-layout" style="width: 100%;height: 100%;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=6" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
    		...
    		<div class="easyui-layout" style="width:100%;height: 100%;">
		    	...
		    	<div data-options="region:'north',border:false" style="height:45%;">
	    				...
	    				<tbody>
	    					<c:forEach items="${otherRoads}" var="otherRoad">
		    					<tr>
		    						...
		    						<td>${otherRoad.firstMosTime}</td>
		    						<td>${otherRoad.mosType}</td>
		    	 ...
		    	 <div data-options="region:'center',border:false" >
		    	 	<div class="easyui-layout" style="width: 100%;height: 100%;">
		    	 		<!-- 语音质差路段优化建议汇总 -->
					    <div data-options="region:'north',border:false" style="height:93px;padding:2px 0px 0px 0px;">
		  					<table id="otherIndexTable" class="easyui-datagrid" data-options="title:'指标呈现',scrollbarSize:0,fit:true,striped:true,fitColumns:true" >
				   				<thead>
				   					<tr>
				   						<th data-options="field:'mosAvg',width:40,align:'center',formatter:numToFixed2Formatter">MOS<br>均值</th>
				   						<th data-options="field:'upRBNum',width:40,align:'center',formatter:numToFixed2Formatter">上行RB<br>数</th>
				   						<th data-options="field:'downRBNum',width:40,align:'center',formatter:numToFixed2Formatter">下行RB<br>数</th>
				   						<th data-options="field:'upMSCAvg',width:60,align:'center',formatter:numToFixed2Formatter">上行MCS<br>均值</th>
				   						<th data-options="field:'downMSCAvg',width:60,align:'center',formatter:numToFixed2Formatter">下行MCS<br>均值</th>
				   						<th data-options="field:'upPHYSpeed',width:80,align:'center',formatter:numToFixed2Formatter">上行PHY速率<br>(kbps)</th>
				   						<th data-options="field:'downPHYSpeed',width:80,align:'center',formatter:numToFixed2Formatter">下行PHY速率<br>(kbps)</th>
				   						<th data-options="field:'upPDCPSpeed',width:90,align:'center',formatter:numToFixed2Formatter">上行PDCP速率<br>(kbps)</th>
				   						<th data-options="field:'downPDCPSpeed',width:90,align:'center',formatter:numToFixed2Formatter">下行PDCP速率<br>(kbps)</th>
				   					</tr>
				   				</thead>
				   				<tbody>
		    						<tr>
				    	...
				    	<!-- 主被叫终端位于不同制式的MOS指标 -->
				    	<div data-options="region:'center',border:false" style="height:34%;padding:2px 0px 0px 0px;">
							<table id="otherCellInfoTable" class="easyui-datagrid" data-options="title:'小区详情列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
				   				<thead>
				   					<tr>
				   						...
				   						<!-- <th data-options="field:'sampleRatio',width:80,align:'center',formatter:numMultiply100ToFixed2Formatter">采样点占整个质差<br>路段采样点比例(%)</th>
				   						<th data-options="field:'cellNumRatio',width:80,align:'center',formatter:numMultiply100ToFixed2Formatter">质差区域采样点与<br>总采样点占比(%)</th> -->
