...
<html>
  <head>
    <title>VoLte异常事件----IMS注册失败问题分析</title>
	...
	var mainTableId = 'imsRegistFailEvent';
	...
	/* 开始分析,分析按钮 */
	function doAnalysis(){
		var imsRegistFailRow = $("#imsRegistFailEvent").datagrid('getSelected');//获取表格中用户选中数据
		if(!imsRegistFailRow){// 操作前至少选中一条
			$.messager.alert("提示","请勾选某个异常事件!",'warning');
			return ;
		}
		testLogItemIds = imsRegistFailRow.recSeqNo;
		volteEEid = imsRegistFailRow.id;
		if(!volteEEid){
			$.messager.alert("提示","请勾选某个异常事件!",'warning');
			return ;
		}
		...
		$.post("${pageContext.request.contextPath}/voiceDropCall/haveTestLogMeasure.action?isOpposite="+false+"&exceptionEventId="+volteEEid+"",
				function(result){
		$("#callAndOtherCall").datagrid('loadData',result);
								}
		,"json"); 
		var roadName = imsRegistFailRow.m_stRoadName;
		//本次和上一次的异常事件id不同
		if(lastVolteEEid!=volteEEid){
			$("#imsRegistFailSignallingTable").datagrid('loadData',[]);
			$("#imsRegistFailSignallingTable").datagrid({
				// 表头
				...
				title:roadName+'IMS注册失败异常事件信令列表',
				fitColumns:true,
				//填满区域
				fit:true,
				//奇偶变色
			...
			$('#imsRegistFailSignallingTable').datagrid('getPanel').addClass("lines-no");
		}
		lastVolteEEid = volteEEid;
		
		$.post("${pageContext.request.contextPath}/voiceQBR/checkDrawCellFileName.action",{roadId:imsRegistFailRow.id},
			function(result){
	...
	<c:if test="${null == imsRegistFails || empty imsRegistFails}">
		isEESortable = false;
	</c:if>
	$(function(){
		<c:if test="${null == imsRegistFails || empty imsRegistFails}">
			$("#imsRegistFailEvent").datagrid('mergeCells',{
				index: 0,
				field: 'boxid',
				colspan:11
			});
		</c:if>
		$("#imsRegistFailSignallingTable").datagrid('mergeCells',{
			index: 0,
			field: 'time',
			colspan:6
		});
  ...
  <body class="easyui-layout" id="mainLayout" style="width: 100%;height: 100%;margin: 0 auto;padding: 0 auto;list-style:none;overflow:hidden;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" style="width: 100%;height: 100%;" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=42" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
    	<div data-options="region:'east',border:false" style="width:55%;padding:0px 0px 0px 2px;">
    		<div class="easyui-layout" style="width:100%;height: 100%;">
		    	...
		    	<div data-options="region:'north',border:false" style="height:45%;">
  					<table id="imsRegistFailEvent" class="easyui-datagrid" data-options="title:'注册失败问题列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,rownumbers:true,singleSelect:true,tools:'#tt3',onLoadSuccess:mainTableLoadSuccess,remoteSort:false,multiSort:true"  >
	    				<thead>
	    					<tr>
	    						...
	    						<th data-options="field:'failDateTime',width:80,align:'center',formatter:showTooltip,sortable:isEESortable">发生时间</th>
	    						<th data-options="field:'failSignallingNode',width:80,align:'center',sortable:isEESortable,
	    							formatter: function(value,row,index){
										if(0==value){
											return '<div title=随机接入 >随机接入</div>';
										}else if(1==value){
											return '<div title=RRC建立 >RRC建立</div>';
										}else if(2==value){
											return '<div title=鉴权 >鉴权</div>';
										}else if(3==value){
											return '<div title=QCI=9的专载建立 >QCI=9的专载建立</div>';
										}else if(4==value){
											return '<div title=QCI=5的专载建立 >QCI=5的专载建立</div>';
										}else if(5==value){
											return '<div title=IMS注册 >IMS注册</div>';
										}
										return value;
									}
	    						">失败信令节点</th>
	    						<th data-options="field:'rsrpAvg',width:60,align:'center',formatter:numToFixed2Formatter,sortable:isEESortable">RSRP<br>均值</th>
	    						<th data-options="field:'rsrpMin',width:60,align:'center',formatter:numToFixed2Formatter,sortable:isEESortable">RSRP<br>最低值</th>
	    						<th data-options="field:'sinrAvg',width:60,align:'center',formatter:numToFixed2Formatter,sortable:isEESortable">SINR<br>均值</th>
	    						<th data-options="field:'sinrMin',width:60,align:'center',formatter:numToFixed2Formatter,sortable:isEESortable">SINR<br>最低值</th>
	    						<th data-options="field:'overlappingCoverageRate',width:60,align:'center',formatter:numToFixed2Formatter,sortable:isEESortable">重叠<br>覆盖率</th>
	    						<th data-options="field:'cellName',width:80,align:'center',formatter:numToFixed2Formatter">服务小区</th>
	    				...
	    				<tbody>
	    					<c:forEach items="${imsRegistFails}" var="imsRegistFail">
		    					<tr>
		    						<td>${imsRegistFail.id}</td>
		    						<td>${imsRegistFail.testLogItem.boxId}</td>
		    						<td>${imsRegistFail.m_stRoadName}</td>
		    						<td>${imsRegistFail.failDateTime}</td>
		    						<td>${imsRegistFail.failSignallingNode}</td>
		    						<td>${imsRegistFail.rsrpAvg}</td>
		    						<td>${imsRegistFail.rsrpMin}</td>
		    						<td>${imsRegistFail.sinrAvg}</td>
		    						<td>${imsRegistFail.sinrMin}</td>
		    						<td>${imsRegistFail.overlappingCoverageRate}</td>
		    						<td>${imsRegistFail.cellName}</td>
		    						<td>${imsRegistFail.testLogItem.fileName}</td>
		    						<td>${imsRegistFail.testLogItem.recSeqNo}</td>
		    						<td>${imsRegistFail.beginLatitude}</td>
		    						<td>${imsRegistFail.courseLatitude}</td>
		    						<td>${imsRegistFail.endLatitude}</td>
		    						<td>${imsRegistFail.beginLongitude}</td>
		    						<td>${imsRegistFail.courseLongitude}</td>
		    						<td>${imsRegistFail.endLongitude}</td>
		    					</tr>
	    					</c:forEach>
	    					<c:if test="${null == imsRegistFails}">
	    						<tr>
		    						<td></td>
		    						<td>未选择测试日志</td>
		    						<td></td>
				...
				<div data-options="region:'center',border:false" style="padding:2px 0px 0px 0px;">
					<table id="imsRegistFailSignallingTable" class="easyui-datagrid" data-options="title:'信令列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
	    				<thead>
	    					<tr>
