define([
    ...
    "esri/renderers/ClassBreaksRenderer"
             ...
             PopupTemplate, GraphicsLayer, ClassBreaksRenderer) {
    return declare([GraphicsLayer], {
        constructor: function (options) {
            ...
            this._zoomEnd = null;
            this._render=options.Renderer || null;
        },
        ...
        onClick: function (e) {
            ...
            if (singles.length > this._maxSingles) {
                alert("抱歉, 聚类显示超过最大点数 " + this._maxSingles + " 个 . 请放大地图查看.");
                return;
            } else {
        ...
        _clusterAddPoint: function (p, cluster) {
            ...
            p.attributes.values = cluster.attributes.values;
        },
        ...
        _addSingles: function (singles) {
            ...
            arrayUtils.forEach(singles, function (p) {
                //判断断点范围
                var values = p.attributes.values;
                var sys;
                if(values<0.5){
                    sys = new SimpleMarkerSymbol("squire",10,null,renderercolor[i].color);
                }
                else{
                    sys = new SimpleMarkerSymbol("circle",10,null,renderercolor[i].color);
                }
                //renderercolor=this._render;
                //for(var i= 0,l=renderercolor.length;i<l;i++){
                //    if(renderercolor[i].end == null){
                //        if (values>renderercolor[i].begin) {
                //            sys = new SimpleMarkerSymbol("circle",10,null,renderercolor[i].color);
                //            break;
                //        }
                //    }
                //    else{
                //        if(renderercolor[i].begin == null){
                //            if (values < renderercolor[i].end) {
                //                sys = new SimpleMarkerSymbol("circle",10,null,renderercolor[i].color);
                //                break;
                //            }
                //        }
                //         else if (values>renderercolor[i].begin && values < renderercolor[i].end) {
                //            sys = new SimpleMarkerSymbol("circle",10,null,renderercolor[i].color);
                //            break;
                //        }
                //    }
