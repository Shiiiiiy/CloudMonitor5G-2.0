...
<html>
  <head>
	...
	var mainTableId = 'dropCallEventTable';
	...
	/* 开始分析,分析按钮 */
	function doAnalysis(){
		var dropCallRow = $("#dropCallEventTable").datagrid('getSelected');//获取表格中用户选中数据
		if(!dropCallRow){// 操作前至少选中一条
			$.messager.alert("提示","请勾选某个异常事件!",'warning');
			return ;
		}
		testLogItemIds = dropCallRow.recSeqNo;
		volteEEid = dropCallRow.id;
		if(!volteEEid){
			$.messager.alert("提示","请勾选某个异常事件!",'warning');
			return ;
		}
		...
		/* $.post("${pageContext.request.contextPath}/videoDropCall/haveTestLogMeasure.action?isOpposite="+true+"&exceptionEventId="+volteEEid+"",
				function(result){
			//data = JSON.parse(result);
		$("#callAndOtherCall").datagrid('loadData',result);
								}
		,"json"); */
		$("#callAndOtherCall").datagrid('load',{
			isOpposite:true,
			exceptionEventId:volteEEid
			
		});
		var roadName = dropCallRow.m_stRoadName;
		//本次和上一次的异常事件id不同
		if(lastVolteEERid!=volteEEid){
			$("#dropCallSignallingTable").datagrid('loadData',[]);
			$("#dropCallOppositeSignallingTable").datagrid('loadData',[]);
			$("#dropCallSignallingTable").datagrid({
				// 表头
				columns:signallingTableColumns,
				url:'${pageContext.request.contextPath}/videoDropCall/doPageListJson.action?isOpposite=false&exceptionEventId='+volteEEid,
				title:roadName+'语音掉话异常事件信令列表',
				fitColumns:true,
				//填满区域
				fit:true,
				//奇偶变色
			...
			$("#dropCallOppositeSignallingTable").datagrid({
				// 表头
				columns:signallingTableColumns,
				url:'${pageContext.request.contextPath}/videoDropCall/doPageListJson.action?isOpposite=true&exceptionEventId='+volteEEid,
				title:roadName+'语音掉话异常事件对端信令列表',
				fitColumns:true,
				//填满区域
				fit:true,
				//奇偶变色
			...
			$('#dropCallSignallingTable').datagrid('getPanel').addClass("lines-no");
			$('#dropCallOppositeSignallingTable').datagrid('getPanel').addClass("lines-no");
		}
		lastVolteEERid = volteEEid;
		
		$.post("${pageContext.request.contextPath}/voiceQBR/checkDrawCellFileName.action",{roadId:dropCallRow.id},
			function(result){
	...
	<c:if test="${null == videoDropCalls || empty videoDropCalls}">
		isEESortable = false;
	</c:if>
	$(function(){
		<c:if test="${null == videoDropCalls || empty videoDropCalls}">
			$("#dropCallEventTable").datagrid('mergeCells',{
				index: 0,
				field: 'boxid',
				colspan:9
			});
		...
		$("#dropCallSignallingTable").datagrid('mergeCells',{
			index: 0,
			field: 'time',
			colspan:6
		});
		$("#dropCallOppositeSignallingTable").datagrid('mergeCells',{
			index: 0,
			field: 'time',
			colspan:6
		});
  ...
  <body class="easyui-layout" id="mainLayout" style="width: 100%;height: 100%;margin: 0 auto;padding: 0 auto;list-style:none;overflow:hidden;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" style="width: 100%;height: 100%;" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=45" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
    	<div data-options="region:'east',border:false" style="width:55%;padding:0px 0px 0px 2px;">
    		<div class="easyui-layout" style="width:100%;height: 100%;">
		    	...
		    	<div data-options="region:'north',border:false" style="height:24%;">
  					<table id="dropCallEventTable" class="easyui-datagrid" data-options="title:'视频掉话问题列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,rownumbers:true,singleSelect:true,tools:'#tt3',onLoadSuccess:mainTableLoadSuccess,remoteSort:false,multiSort:true"  >
	    				<thead>
	    					<tr>
	    						...
	    						<th data-options="field:'failDateTime',width:80,align:'center',formatter:showTooltip,sortable:isEESortable">掉话时间</th>
								...
								">主/被叫</th>
	    						<th data-options="field:'dropCallRsrp',width:60,align:'center',formatter:numToFixed2Formatter,sortable:isEESortable">掉话时<br>RSRP</th>
	    						<th data-options="field:'dropCallSinr',width:60,align:'center',formatter:numToFixed2Formatter,sortable:isEESortable">掉话时<br>SINR</th>
	    						<th data-options="field:'overlappingCoverageRate',width:60,align:'center',formatter:numToFixed2Formatter,sortable:isEESortable">重叠<br>覆盖度</th>
	    				...
	    				<tbody>
	    					<c:forEach items="${videoDropCalls}" var="dropCall">
		    					<tr>
		    						<td>${dropCall.id}</td>
		    						<td>${dropCall.testLogItem.boxId}</td>
		    						<td>${dropCall.m_stRoadName}</td>
		    						<td>${dropCall.failDateTime}</td>
		    						<td>${dropCall.testLogItem.callType}</td>
		    						<td>${dropCall.dropCallRsrp}</td>
		    						<td>${dropCall.dropCallSinr}</td>
		    						<td>${dropCall.overlappingCoverageRate}</td>
		    						<td>${dropCall.cellName}</td>
		    						<td>${dropCall.testLogItem.fileName}</td>
		    						<td>${dropCall.testLogItem.recSeqNo}</td>
		    						<td>${dropCall.beginLatitude}</td>
		    						<td>${dropCall.courseLatitude}</td>
		    						<td>${dropCall.endLatitude}</td>
		    						<td>${dropCall.beginLongitude}</td>
		    						<td>${dropCall.courseLongitude}</td>
		    						<td>${dropCall.endLongitude}</td>
		    					</tr>
	    					</c:forEach>
	    					<c:if test="${null == videoDropCalls}">
	    						<tr>
		    						<td></td>
		    						<td>未选择测试日志</td>
		    						<td></td>
				...
				<div data-options="region:'center',border:false" style="padding:2px 0px 0px 0px;">
					<table id="dropCallSignallingTable" class="easyui-datagrid" data-options="title:'信令列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
	    				<thead>
	    					<tr>
				...
				<div data-options="region:'south',border:false"  style="height:38%;padding:2px 0px 0px 0px;">
					<table id="dropCallOppositeSignallingTable" class="easyui-datagrid" data-options="title:'对端信令列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
	    				<thead>
	    					<tr>
	    						<th data-options="field:'time',width:120,align:'center'">Time</th>
	    						<th data-options="field:'dir',width:50,align:'center'">Dir</th>
				...
				<div id="areaInfoDlg" class="easyui-dialog" style="width:70%;height:50%;" data-options="title:'小区测量',closed:true,modal:true">
				<table id="callAndOtherCall"class="easyui-datagrid"style="width:100%;height:100%;" data-options="url:'${pageContext.request.contextPath}/videoDropCall/haveTestLogMeasure.action',scrollbarSize:0,fit:true,striped:true,fitColumns:true,rownumbers:true,pagination:true,pageList:[100,500,1000,2000,3000,5000],pageSize:100">
					<thead>
	    					<tr>
