...
public abstract class ClassUtils {
	 ...
	 * @param className
	 *            类名
	 * @return 如果是数组类，返回<code>true</code>，否则返回 <code>false</code>
	 */
	 ...
	 *            类
	 */
	private static void addInterfaces(Set set, Class clazz) {
		if (clazz != null && clazz != Object.class) {
			if (clazz.isInterface() && set.contains(clazz) == false) {
				set.add(clazz);
			}
			...
			if (interfaces != null && interfaces.length > 0) {
				for (int c = 0; c < interfaces.length; c++) {
					...
					if (set.contains(interfaceClass) == false) {
						set.add(interfaceClass);
					}
					addInterfaces(set, interfaceClass);
				}
			}
			addInterfaces(set, clazz.getSuperclass());
		}
	}
	...
	public static Class<?> loadClass(String className)
		...
		return loader != null ? loader.loadClass(className) : Class
				.forName(className);
	}
