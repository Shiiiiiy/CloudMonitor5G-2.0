...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function beginUpload(){
			if($("#saveForm").form('validate')){
				...
				var moduleNo = $("#moduleNo").combobox('getValue');
				var fileLogType =  $("#fileLogType").combobox('getValue');
				config.postVarsPerFile.boxId=boxId;
				config.postVarsPerFile.moduleNo=moduleNo;
				config.postVarsPerFile.testFileType=fileLogType;
				_t.upload();
			}
		}
		/**
		...
		var config = {
			...
			maxSize: 104857600,//100M, /** 单个文件的最大大小，默认:2G */
	...
	//			param2: "val2"
			...
			tokenURL : "${pageContext.request.contextPath}/networkTestLogItem/token.action", /** 根据文件名、大小等信息获取Token的URI（用于生成断点续传、跨域的令牌） */
		//	frmUploadURL : "${pageContext.request.contextPath}/uploadFile/formupload.servlet", /** Flash上传的URI */
			frmUploadURL : "${pageContext.request.contextPath}/networkTestLogItem/formUploadFile.action?", /** Flash上传的URI */
			//uploadURL : "http://localhost:8080/mos/file/upload.action?myfileType=sampleFile", /** HTML5上传的URI */
			uploadURL : "${pageContext.request.contextPath}/networkTestLogItem/html5UploadFile.action", /** HTML5上传的URI */
			simLimit:100, /** 单次最大上传文件个数 */
			extFilters: [".pcap",".PCAP"], /** 允许的文件扩展名, 默认: [] */
	...
	//		onCancel : function(file) {alert('Canceled:  ' + file.name)}, /** 取消上传文件的响应事件 */
			...
			onQueueComplete: function() {
				...
				$.messager.alert('提示','所有文件上传完成','info',function(){
					goToPage('${pageContext.request.contextPath}/networkTestLogItem/networkTestLogItemListUI.action');
				});
				//_t.showBrowseBlock();
				//_t.destroy();
			...
			onUploadError: function(status, msg) {
				var objMsg = eval("("+msg+")");
				fShowMessage1("上传错误.响应状态:" + status + ", 错误提示: " + objMsg.message, true);
			} /** 文件上传出错的响应事件 */
		};
	...
	/* 加载模块下拉框 */
	function changeModuleNo(newValue,oldValue){
		$("#moduleNo").combobox({
			url:'${pageContext.request.contextPath}/terminal/terminalModuleNoTree.action?boxId='+newValue,
			method: 'post',
			panelWidth:300,
			editable:false,
			multiple:false,
    	...
    	<div style="width:500px;" data-options="region:'east',split:false,border:false">
	    	<form id="saveForm" method="post" enctype="multipart/form-data">
		    	<table>
		    		...
		    		<tr style="height:30px;">
		    			<td width="100px" align="right">
			    			<span class="label" style="font-size:12px;line-height:26px;">通道号：</span>
						</td>
						<td width="300px" align="left">
							<select id="moduleNo" style="width:300px;" name="moduleNo" data-options="required:true,editable:false" class="easyui-combobox"  ></select>
						</td>
		    		</tr>
		    		<tr style="height:30px;">
		    			<td width="100px" align="right">
			    			<span class="label" style="font-size:12px;line-height:26px;">接口：</span>
						</td>
						<td width="300px" align="left">
							<select id="fileLogType" style="width:300px;" name="fileLogType" data-options="required:true,editable:false"  class="easyui-combobox"  >
								<option value="0">S1</option>
								<option value="1">SGI</option>
							</select>
						</td>
		    		</tr>
		    		<tr style="height:30px;">
