...
$(function(){
	...
	if(null!=cityIds){
	$.post("${pageContext.request.contextPath}/report/getCityInfo.action?cityIds="+cityIds+"",
			function(result){
				if (result.errorMsg) {
	...
	if(null!=logIds){
	$.post("${pageContext.request.contextPath}/report/getTestLogItem.action?logIds="+logIds+"",
			function(result){
				if (result.errorMsg) {
					$.messager.alert("系统提示", result.errorMsg,'error');
				} else {
		...
		$("#reportForm").form('submit',{
			...
			success : function(result) {
				...
				if (resultData.errorMsg) {
					...
					goToPage('${pageContext.request.contextPath}/report/goSee.action');
				}
			}
		});
	}
	...
	function goToTestPlanList(){
		goToPage('${pageContext.request.contextPath}/report/listUI.action');
	}
	//打开添加域窗口
	function addATU(){
		$('#winATU').window('open');
	...
	function addBox(){
		...
		$.post("${pageContext.request.contextPath}/report/terminalTree.action?cityIds="+cityIdsString+"",
				function(result){
					if (result.errorMsg) {
						$.messager.alert("系统提示", result.errorMsg,'error');
					} else {
	...
	function addLog(){
		...
		$.post("${pageContext.request.contextPath}/report/testLogItem.action?cityIds="+cityIdsString+"&boxIds="+boxIdsString+"&beginDate="+beginDate+"&endDate="+endDate+"&testRank="+testRankString+"",
				function(result){
					if (result.errorMsg) {
						$.messager.alert("系统提示", result.errorMsg,'error');
					} else {
	 ...
	 <div id="ly" class="easyui-layout"  style="width:100%;height: 100%;">
		<div data-options="region:'north',border:false,title:'新增统计任务，带*为必填项'"style="height:560px;overflow-y:auto;overflow-x:hidden;" >
			<div style="padding:0px 10px;">
				<form id="reportForm" style="margin:0px;" method="post" action="${pageContext.request.contextPath}/report/addReportTask.action" >
						<div class="listTitle">统计任务内容</div>
						<div class="testPlan_tabs">
						<div class="inputDivShow">
							<font color="red">*</font>
							统计任务名称
							<input class="easyui-textbox" data-options="required:true,readonly:true" name="statisticeTaskRequest.name" <c:if test="${null==statisticeTask.name}">value="<域名>-<开始时间>-<结束时间>"</c:if>value="${statisticeTask.name}"  id="name">
						</div>
						<br>
						<div class="inputDivShow">
							<font color="red">*</font>
						...
						<br>
						<div class="inputDivShow" style="overflow: auto;width:36%">
			    	...
			    	<div id="winATU" class="easyui-dialog" buttons="#atu-buttons"title="添加ATU域" style="width:600px;height:400px;overflow: auto;position:relative;"data-options="onMove:onDrag,collapsible:false,minimizable:false,maximizable:false,modal:true,closed:true">
				    	<ul id="areaTree" name="statisticeTaskRequest.cityIds"  class="easyui-tree" multiple="multiple" data-options="onLoadSuccess:open,panelWidth:200,editable:false,onlyLeafCheck:true,multiple:true,lines:true,checkbox:true,url:'${pageContext.request.contextPath}/terminalGroup/terminalGroupTree.action'"  >
			    		</ul>
			    		<div id="atu-buttons">
			    		...
			    		<table  id="boxTab2"class="easyui-datagrid" >
    						<thead>
								<tr>
									...
									<th data-options="field:'testTarget',width:160,align:'center'">终端类型</th>
			    	...
			    	<div id="winLog" class="easyui-dialog"  title="添加Log文件" style="width:600px;height:400px;position:relative;"data-options="buttons:'#log-buttons',onMove:onDrag,collapsible:false,minimizable:false,maximizable:false,modal:true,closed:true">
				    	<table  id="logTab2"class="easyui-datagrid"style="width:597px;height:345px">
    						<thead>
								<tr>
									<th data-options="field:'',checkbox:true"></th>
						...
						<div id="log-buttons"style="width:597px;height: 15px ">
