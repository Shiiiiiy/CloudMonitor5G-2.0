...
<html>
  <head>
	...
	var mainTableId = 'cdfbFailEventTable';
	...
	/* 开始分析,分析按钮 */
	function doAnalysis(){
		var csfbFailRow = $("#cdfbFailEventTable").datagrid('getSelected');//获取表格中用户选中数据
		if(!csfbFailRow){// 操作前至少选中一条
			$.messager.alert("提示","请勾选某个异常事件!",'warning');
			return ;
		}
		testLogItemIds = csfbFailRow.recSeqNo;
		volteEEid = csfbFailRow.id;
		if(!volteEEid){
			$.messager.alert("提示","请勾选某个异常事件!",'warning');
			return ;
		}
		...
		,"json");
		var roadName = csfbFailRow.m_stRoadName;
		//本次和上一次的异常事件id不同
		if(lastVolteEERid!=volteEEid){
			$("#csfbFailSignallingTable").datagrid('loadData',[]);
			$("#csfbFailOppositeSignallingTable").datagrid('loadData',[]);
			$("#csfbFailSignallingTable").datagrid({
				// 表头
				columns:signallingTableColumns,
				url:'${pageContext.request.contextPath}/csfbFail/doPageListJson.action?isOpposite=false&exceptionEventId='+volteEEid,
				title:roadName+'CSFB失败异常事件信令列表',
				fitColumns:true,
				//填满区域
				fit:true,
				//奇偶变色
			...
			$('#csfbFailSignallingTable').datagrid('getPanel').addClass("lines-no");
			$('#csfbFailOppositeSignallingTable').datagrid('getPanel').addClass("lines-no");
		}
		lastVolteEERid = volteEEid;
		
		$.post("${pageContext.request.contextPath}/voiceQBR/checkDrawCellFileName.action",{roadId:csfbFailRow.id},
			function(result){
	...
	<c:if test="${null == csfbFails || empty csfbFails}">
		isEESortable = false;
	</c:if>
	$(function(){
		<c:if test="${null == csfbFails || empty csfbFails}">
			$("#cdfbFailEventTable").datagrid('mergeCells',{
				index: 0,
				field: 'boxid',
				colspan:7
			});
		...
		$("#csfbFailSignallingTable").datagrid('mergeCells',{
			index: 0,
			field: 'time',
			colspan:6
		});
		$("#csfbFailOppositeSignallingTable").datagrid('mergeCells',{
			index: 0,
			field: 'time',
			colspan:6
		});
  ...
  <body class="easyui-layout" id="mainLayout" style="width: 100%;height: 100%;margin: 0 auto;padding: 0 auto;list-style:none;overflow:hidden;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" style="width: 100%;height: 100%;" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=43" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
    	<div data-options="region:'east',border:false" style="width:55%;padding:0px 0px 0px 2px;">
    		<div class="easyui-layout" style="width:100%;height: 100%;">
		    	...
		    	<div data-options="region:'north',border:false" style="height:24%;">
  					<table id="cdfbFailEventTable" class="easyui-datagrid" data-options="title:'CSFB失败问题列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,rownumbers:true,singleSelect:true,tools:'#tt3',onLoadSuccess:mainTableLoadSuccess,remoteSort:false,multiSort:true"  >
	    				<thead>
	    					<tr>
	    						...
	    						<th data-options="field:'cellName',width:80,align:'center',sortable:isEESortable">服务小区</th>
	    				...
	    				<tbody>
	    					<c:forEach items="${csfbFails}" var="csfbFail">
		    					<tr>
		    						<td>${csfbFail.id}</td>
		    						<td>${csfbFail.testLogItem.boxId}</td>
		    						<td>${csfbFail.m_stRoadName}</td>
		    						<td>${csfbFail.failDateTime}</td>
		    						<td>${csfbFail.rsrpAvg}</td>
		    						<td>${csfbFail.sinrAvg}</td>
		    						<td>${csfbFail.cellName}</td>
		    						<td>${csfbFail.testLogItem.fileName}</td>
		    						<td>${csfbFail.testLogItem.recSeqNo}</td>
		    						<td>${csfbFail.beginLatitude}</td>
		    						<td>${csfbFail.courseLatitude}</td>
		    						<td>${csfbFail.endLatitude}</td>
		    						<td>${csfbFail.beginLongitude}</td>
		    						<td>${csfbFail.courseLongitude}</td>
		    						<td>${csfbFail.endLongitude}</td>
		    					</tr>
	    					</c:forEach>
	    					<c:if test="${null == csfbFails}">
	    						<tr>
		    						<td></td>
		    						<td>未选择测试日志</td>
		    						<td></td>
				...
				<div data-options="region:'center',border:false" style="padding:2px 0px 0px 0px;">
					<table id="csfbFailSignallingTable" class="easyui-datagrid" data-options="title:'信令列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
	    				<thead>
	    					<tr>
				...
				<div data-options="region:'south',border:false"  style="height:38%;padding:2px 0px 0px 0px;">
					<table id="csfbFailOppositeSignallingTable" class="easyui-datagrid" data-options="title:'对端信令列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
	    				<thead>
	    					<tr>
	    						<th data-options="field:'time',width:120,align:'center'">Time</th>
	    						<th data-options="field:'dir',width:50,align:'center'">Dir</th>
