  ...
  <head>
	...
	<script type="text/javascript">
		...
		var testLogInfoColumns=[[
			{field:'name',width:60,align:'center',title:'车载终端'}, 
			{field:'time',width:90,align:'center',title:'命令执行时间'},
			{field:'commandDirection',width:45,align:'center',title:'上行/下行'},
			{field:'commandName',width:60,align:'center',title:'命令名称'},
			{field:'commandAccessaryParameters',width:150,align:'center',title:'命令附带参数'}
		]];
		$(function(){
		...
		function initTable(){
			$("#testLogInfoTable").datagrid({
				...
				url:'${pageContext.request.contextPath}/commandInteract/doPageListJson.action',
				title:'命令交互监控列表',
				fitColumns:true,
				//填满区域
				fit:true,
		...
		function query(){
			if($("#queryForm").form('validate')){
				...
				$("#testLogInfoTable").datagrid('load',{
					...
					boxIds:boxIdsString,
					commandName:$("#commandName").textbox('getValue'),
    	...
    	<div id="ly" class="easyui-layout"  style="width:100%;height: 100%;padding:4px;top:4px;left:4px;position: absolute;">
	    	...
	    	<div data-options="region:'north',title:'筛选条件设置',onCollapse:function(){
    			...
    			<form id="queryForm" class="esyui-form" method="post" style="width:100%;height: 100%;margin:0;" >
			    	...
			    	<div class="inputDivShow">命令名称
			    		<input id="commandName" name="commandName"  class="easyui-textbox" data-options="validType:'length[1,30]'"/>
			    	</div>
