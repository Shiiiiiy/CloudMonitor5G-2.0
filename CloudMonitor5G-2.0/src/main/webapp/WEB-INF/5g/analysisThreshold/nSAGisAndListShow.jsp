...
<html>
<head>
    <title>NSA模式异常事件分析地图和列表展示</title>
    ...
    <script type="text/javascript">
        ...
        var testLogInfoColumns=
        	[[
        		...
        		{field:'lteN',align:'center',colspan:7,title:'LTE锚点小区信息'},
        		{field:'nrC',align:'center',colspan:7,title:'NR辅小区信息'},
        		{field:'lteC',align:'center',colspan:4,title:'LTE目标小区信息'},
        		{field:'nrC',align:'center',colspan:6,title:'NR目标小区信息'}
        	],[
        		{field:'id',width:40,align:'center',title:'序号',hidden:true}, // 一个对象代表一列 <th>
        		{field:'city',width:150,align:'center',title:'城市',sortable:true},
	            {field:'scellChangeReqStartTime',width:150,align:'center',title:'Scell Change请求发起时间',sortable:true},
	            {field:'scellChangeFailTime',width:150,align:'center',title:'失败发生时间',sortable:true},
	            {field:'scellChangeFailTimeLong',width:150,align:'center',title:'Scell Change失败发生时间时间戳',sortable:true,hidden:true},
	            {field:'fileName',width:100,align:'center',title:'文件名称',sortable:true,
	                formatter:function(value,row,index){
	                     /* if(row.fileName){
	                     	var filename = value.substring(value.lastIndexOf("/")+1,value.length );
	                        //return '<a title="" href="${pageContext.request.contextPath}/nasexceptionview/downloadFile.action?fileId='+row.id+'&reqSeqNo='+row.testLogItem.recSeqNo+'" >'+filename+'</a>';
					     } */
	                    return value;
	                }
	            },
	            //{field:'roadName',width:100,align:'center',title:'道路名称',sortable:true,hidden:true},
	            {field:'longitude',width:75,align:'center',title:'经度',sortable:true},
	            {field:'dimension',width:75,align:'center',title:'纬度',sortable:true},
	            
	            {field:'scellChangeFailType',width:100,align:'center',title:'失败类型',sortable:true},
	            {field:'wirelessFailReason',width:100,align:'center',title:'无线原因分析',sortable:true,id:"wirelessFailReason_val"},
	            
	            {field:'dlIbler',width:60,align:'center',title:'DL_ibler',sortable:true},
	            {field:'lteTac',width:60,align:'center',title:'TAC',sortable:true},
	            {field:'lteCellid',width:100,align:'center',title:'CELLID',sortable:true},
	            {field:'lteFriendlyName',width:100,align:'center',title:'友好名',sortable:true},
	            {field:'lteFrequencyPoint',width:100,align:'center',title:'频点',sortable:true},
	            {field:'ltePci',width:100,align:'center',title:'PCI',sortable:true},
	            {field:'ltePccRsrp',width:60,align:'center',title:'LTE PCC RSRP',sortable:true},
	            
	            {field:'nrDlIbler',width:60,align:'center',title:'DL_ibler(%)',sortable:true},
	            {field:'nrTac',width:60,align:'center',title:'TAC',sortable:true},
	            {field:'nrCellid',width:60,align:'center',title:'CELLID',sortable:true},
	            {field:'nrFriendlyName',width:100,align:'center',title:'友好名',sortable:true},
	            {field:'nrFrequencyPoint',width:100,align:'center',title:'频点',sortable:true},
	            {field:'nrPci',width:60,align:'center',title:'PCI',sortable:true},
	            {field:'nrPccRsrp',width:60,align:'center',title:'NR RSRP',sortable:true},
	            
	            {field:'lteTargetCellTac',width:60,align:'center',title:'LTE目标小区TAC',sortable:true},
	            {field:'lteTargetCellCellid',width:60,align:'center',title:'LTE目标小区CELLID',sortable:true},
	            {field:'lteTargetCellFrequency',width:100,align:'center',title:'LTE目标小区频点',sortable:true},
	            {field:'lteTargetCellPci',width:100,align:'center',title:'LTE目标小区PCI',sortable:true},
	            //{field:'lteTargetCellSsRsrp',width:45,align:'center',title:'LTE目标小区ss-rsrp',sortable:true},
	            
	            {field:'nrTargetCellTac',width:100,align:'center',title:'NR目标小区TAC',sortable:true},
	            {field:'nrTargetCellCellid',width:60,align:'center',title:'NR目标小区CELLID',sortable:true},
	            {field:'nrTargetCellFrequency',width:100,align:'center',title:'NR目标小区频点',sortable:true},
	            {field:'nrTargetCellPci',width:100,align:'center',title:'NR目标小区PCI',sortable:true}
	           // , {field:'nrTargetCellSsRsrp',width:90,align:'center',title:'NR目标小区ss-rsrp',sortable:true}
           ]
        ];
        ...
        function downloadLog(){
        	var rows = $("#testLogInfoTable").datagrid('getSelections');
			if(rows.length !=1){// 
				$.messager.alert("提示","请选择一条记录!",'warning');
				return ;
			}
			var fileId = rows[0].id;
			var reqSeqNo = rows[0].testLogItem.recSeqNo;
        	window.location= "${pageContext.request.contextPath}/nasexceptionview/downloadFile.action?fileId="+fileId+"&reqSeqNo="+reqSeqNo;
        }
        ...
        function getGpsPointData(recSeqNo,timeLong,typeName,lon,lat){
        	$.ajax({
                type:"GET",
                url:"${pageContext.request.contextPath}/nasexceptionview/getGpsPointData.action",
                data:{
                	'recSeqNo':recSeqNo,
                	'timeLong':timeLong
					},
        ...
        var query = function(){
            ...
            $("#testLogInfoTable").datagrid({
                // 表头
                columns:testLogInfoColumns,
                url:'${pageContext.request.contextPath}/nasexceptionview/doPageListJson.action?nrFriendlyName='+roadName,
                title:'NSA模式异常事件列表',
                fitColumns:false,
                //填满区域
                fit:true,
...
<!--地图界面 -->
	...
	<div id="toolbar">
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-download" plain="true" onclick="downloadLog();">导出</a>
	</div>
