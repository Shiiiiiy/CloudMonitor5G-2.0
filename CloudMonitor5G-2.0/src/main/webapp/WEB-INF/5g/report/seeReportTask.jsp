...
/* 	var templateIds='${statisticeTask.templateIds}';
		...
		$("#reportForm").form('submit',{
			...
			success : function(result) {
				...
				if (resultData.errorMsg) {
					...
					if(dpage=='0'){
						parent.addTab(null,"${pageContext.request.contextPath}/report5g/goSee.action","报表统计",null);
						/* goToPage('${pageContext.request.contextPath}/report5g/goSee.action'); */
					}else if(dpage=='1'){
						parent.addTab(null,"${pageContext.request.contextPath}/report5g/goTraii.action","测试轨迹统计",null);
						/* goToPage('${pageContext.request.contextPath}/report5g/goTraii.action'); */
					}
				}
			}
		});
	...
	function goToTestPlanList(){
		goToPage('${pageContext.request.contextPath}/report5g/listUI.action');
	}
	//打开添加域窗口
	function addATU(){
		$('#winATU').window('open');
	...
	function addLog(){
		...
		$.post("${pageContext.request.contextPath}/report5g/testLogItem.action?cityIds="+cityIdsString+"&boxIds="+boxIdsString+"&beginDate="+beginDate+"&endDate="+endDate+"&testRank="+testRankString+"",
				function(result){
					if (result.errorMsg) {
						$.messager.alert("系统提示", result.errorMsg,'error');
					} else {
	...
	function addTemplate(){
		...
		$.post("${pageContext.request.contextPath}/report5g/regionReportTemplate.action?cityIds="+cityIdsString+"",
				function(result){
					if (result.errorMsg) {
						$.messager.alert("系统提示", result.errorMsg,'error');
					} else {
						$("#templateTab2").datagrid('loadData',result.regionReportTemplate);
					}
				}
			,"json");
	}
	 ...
	 <div id="ly" class="easyui-layout"  style="width:100%;height: 100%;">
		<div data-options="region:'center',border:false,title:'新增统计任务，带*为必填项'"style="overflow-y:auto;overflow-x:hidden;" >
			<div style="padding:0px 10px;">
				<form id="reportForm" style="margin:0px;" method="post" action="${pageContext.request.contextPath}/report5g/seeReportTask.action" >
		   	...
		   	<table width="100%" style="border-top:1px solid #95b8e7;">
		   		<tr height="35px">
		    		<td width="50%;" align="right">
		    			<a id="saveButton" iconCls="icon-ok" class="easyui-linkbutton" style="width:100px;" onclick="seeReport();"  >查看</a>
		    		</td>
		    		<td width="50%;">
