				...
				columns:[[
					...
					{field:'nam',width:80,align:'center',title:'查看详情',
						formatter:function(value,row,index){
							/* if(row.taskStatus == 2 || row.taskStatus == null){
					...
					{field:'trail',width:80,align:'center',title:'测试轨迹',
						formatter:function(value,row,index){
							/* if(row.taskStatus == 2 || row.taskStatus == null){
								return '<a href="#" title="测试轨迹" onclick="seeTestTrail('+row.id+');" >测试轨迹</a>';
							}else{
								return '<a style="opacity: 0.2" href="javascript:return false;" title="测试轨迹" onclick="return false;" >测试轨迹</a>';
							} */
							return '<a href="#" title="测试轨迹" onclick="seeTestTrail('+row.id+');" >测试轨迹</a>';
						}
					},
...
/* 					{field:'na',width:40,align:'center',title:'移动',
				...
				url:'${pageContext.request.contextPath}/report5g/doPageListJson.action',
				title:'统计任务列表',
				fitColumns:true,
				//填满区域
				fit:true,
				...
				onLoadSuccess: function(data){//加载完毕后获取所有的checkbox遍历
		            if (data.rows.length > 0) {
		                ...
		                for (var i = 0; i < data.rows.length; i++) {
		                    //根据operate让某些行不可选
		                    if (2 != data.rows[i].taskStatus && null != data.rows[i].taskStatus) {
		...
		/*查看详情*/
		function seeInfo(id){
			goToPage("${pageContext.request.contextPath}/report5g/seeInfo.action?idLong="+id+"&dPage=0");
		}
		/*测试轨迹*/
		function seeTestTrail(id){
			goToPage("${pageContext.request.contextPath}/report5g/seeInfo.action?idLong="+id+"&dPage=1");
		}
		/* 跳转到添加任务页面 */
		function add(){
			goToPage('${pageContext.request.contextPath}/report5g/goAdd.action?dPage=0');
		}
		/* 删除任务*/
		function delReport(){
			...
			if (row.length!=0) {
				$.messager.confirm("系统提示", "确定要删除选中任务？", function(r) {
					if (r) {
						...
						$.post('${pageContext.request.contextPath}/report5g/delReport.action', {ids : idsString}, 
							function(result) {
								if (result.errorMsg) {
