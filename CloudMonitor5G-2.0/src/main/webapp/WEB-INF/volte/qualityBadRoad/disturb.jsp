...
<html>
  <head>
	...
	function doAnalysis(){
		...
		// 以ajax方式，发送到服务器，完成操作
		$.post("${pageContext.request.contextPath}/voiceQBR/doDisturbRoadAnalysis.action",{roadId:disturbRow.id},
			function(result){
				if (result.errorMsg) {
					...
					//指标列表
					if(disturbRoadName){
						$("#disturbIndexTable").datagrid('getPanel').panel('setTitle',disturbRoadName+"路段"+$("#disturbIndexTable").datagrid('options').title);
					}
					$("#disturbIndexTable").datagrid('loadData',result.disturbRoadIndex);
	...
	$(function(){
		...
		$("#disturbIndexTable").datagrid('mergeCells',{
			index: 0,
			field: 'callType',
			colspan:8
		});
  ...
  <body id="mainLayout" class="easyui-layout" style="width: 100%;height: 100%;margin: 0 auto;padding: 0 auto;list-style:none;overflow:hidden;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=2" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
    	<div data-options="region:'east',border:false" style="width:55%;padding:0px 0px 0px 2px;">
    		<div class="easyui-layout" style="width:100%;height: 100%;">
		    	...
		    	<div data-options="region:'north',border:false" style="height:33%;">
  					<table id="disturbRoadTable" class="easyui-datagrid" data-options="title:'干扰原因质差路段列表',scrollbarSize:0,fit:true,striped:true,fitColumns:true,rownumbers:true,singleSelect:true,tools:'#tt3',onLoadSuccess:mainTableLoadSuccess,remoteSort:false,multiSort:true"  >
	    				<thead>
	    					<tr>
		    	...
		    	<div data-options="region:'center',border:false" >
					<div class="easyui-layout" style="width:100%;height: 100%;">
						<div data-options="region:'north',border:false" style="height:106px;padding:2px 0px 0px 0px;">
							<table  id="disturbIndexTable" class="easyui-datagrid" data-options="scrollbarSize:0,fit:true,striped:true,fitColumns:true,title:'指标列表'" >
			    				<thead>
			    					<tr>
			    						<th rowspan="2" data-options="field:'callType',width:40,align:'center'">主/被<br>叫</th>
			    						<th rowspan="2" data-options="field:'mosAvg',width:80,align:'center',formatter:numToFixed2Formatter">MOS均值</th>
			    						<th colspan="2" >码字0BLER</th>
			    						<th colspan="2" >码字1BLER</th>
			    						<th rowspan="2" data-options="field:'rtpDropRatio',width:80,align:'center',formatter:numToFixed2Formatter">RTP丢包率</th>
			    						<th rowspan="2" data-options="field:'rtpShake',width:80,align:'center',formatter:numToFixed2Formatter">RTP抖动(ms)</th>
			    					</tr>
			    					<tr>
			    						<th data-options="field:'codon0FirstBler',width:80,align:'center',formatter:numToFixed2Formatter">初始BLER</th>
			    						<th data-options="field:'codon0ResidualBler',width:80,align:'center',formatter:numToFixed2Formatter">残留BLER</th>
			    						<th data-options="field:'codon1FirstBler',width:80,align:'center',formatter:numToFixed2Formatter">初始BLER</th>
			    						<th data-options="field:'codon1ResidualBler',width:80,align:'center',formatter:numToFixed2Formatter">残留BLER</th>
			    					</tr>
			    				</thead>
			    				<tbody>
		    						<tr>
									...
									<table id="disturbAutoTable" class="easyui-datagrid" data-options="title:'优化建议:三超小区详情',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
					    				<thead>
					    					<tr>
					    						...
					    						<!-- <th data-options="field:'itemid5',width:120,align:'center'">采样点占整个质差路段<br>采样点比例(%)</th> -->
					    					</tr>
					    				</thead>
					    				<tbody>
				    						<tr>
