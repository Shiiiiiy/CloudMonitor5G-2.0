...
<html>
  <head>
	...
	//获取及回显道路名称
	var mainTableId = 'patternSwitchTable';
	...
	/* 开始分析,分析按钮 */
	function doAnalysis(){
		var patternSwitchRow = $("#patternSwitchTable").datagrid('getSelected');//获取表格中用户选中数据
		if(!patternSwitchRow){// 操作前至少选中一条
			$.messager.alert("提示","请勾选某个质差问题!",'warning');
			return ;
		}
		testLogItemIds = patternSwitchRow.recSeqNo;
		volteqbrid = patternSwitchRow.id;
		if(!volteqbrid){
			$.messager.alert("提示","请勾选某个质差问题!",'warning');
			return ;
		}
		// 以ajax方式，发送到服务器，完成操作
		$.post("${pageContext.request.contextPath}/videoQuality/doPatternSwitchAnalysis.action",{roadId:patternSwitchRow.id},
			function(result){
				if (result.errorMsg) {
					$.messager.alert("系统提示", result.errorMsg,'error');
				} else {
					$("#patternSwitchAdviceTable").datagrid('loadData',result.patternSwitchAdjust);
					$("#patternSwitchKeyIndexTable").datagrid('loadData',result.patternSwitchKeyIndex);
				}
			}
		,"json");
		$.post("${pageContext.request.contextPath}/videoQuality/checkDrawCellFileName.action",{roadId:patternSwitchRow.id},
			function(result){
	...
	<c:if test="${null == patternSwitchs || empty patternSwitchs}">
		isRoadSortable = false;
	</c:if>
	$(function(){
		<c:if test="${null == patternSwitchs || empty patternSwitchs}">
			$("#patternSwitchTable").datagrid('mergeCells',{
				index: 0,
				field: 'boxId',
				colspan:10
			});
		</c:if>
		$("#patternSwitchAdviceTable").datagrid('mergeCells',{
			index: 0,
			field: 'cellName',
			colspan:6
		});
		$("#patternSwitchKeyIndexTable").datagrid('mergeCells',{
			index: 0,
			field: 'videoBitRate',
			colspan:5
		});
  ...
  <body id="mainLayout" class="easyui-layout" style="width: 100%;height: 100%;margin: 0 auto;padding: 0 auto;list-style:none;overflow:hidden;">
    	...
    	<div  data-options="region:'center',border:false" style="padding:0px 2px 0px 0px;">
    		<div class="easyui-panel" data-options="fit:true">
    			<iframe name="mapIframe" src="${pageContext.request.contextPath}/gis/default.html?toolbarType=37" scrolling="auto" frameborder="0"  style="width:100%;height:100%;border:0;margin: 0;" ></iframe>
    		</div>
    	</div>
    	<!-- 东,指标列表和汇总统计信息 -->
    	<div data-options="region:'east',border:false" style="width:55%;padding:0px 0px 0px 2px;">
    		<div class="easyui-layout" style="width:100%;height: 100%;">
		    	...
		    	<div data-options="region:'north',border:false" style="height:33%;">
	    			<table id="patternSwitchTable" class="easyui-datagrid" data-options="title:'问题列表',onLoadSuccess:mainTableLoadSuccess,scrollbarSize:0,fit:true,striped:true,fitColumns:true,rownumbers:true,singleSelect:true,tools:'#tt3',remoteSort:false,multiSort:true"  >
	    				<thead>
	    					<tr>
	    						...
	    						">关键参<br>数原因</th>
	    						<th data-options="field:'cellName',width:60,align:'center',formatter:showTooltip">服务小<br>区</th>
	    						<th data-options="field:'beforeSwitchTM',width:90,align:'center',formatter:showTooltip">转换前TM</th>
	    						<th data-options="field:'afterSwitchTM',width:90,align:'center',formatter:showTooltip">转换后TM</th>
	    						<th data-options="field:'optimization',width:120,align:'center',formatter:showTooltip">优化建议</th>
	    						<th data-options="field:'fileName',width:100,align:'center',formatter:showTooltip">测试日志<br>名称</th>
	    						<th data-options="field:'recSeqNo',width:20,align:'center',hidden:true">测试日志ID</th>
	    				...
	    				<tbody>
	    					<c:forEach items="${patternSwitchs}" var="patternSwitch">
		    					<tr>
		    						<td>${patternSwitch.id}</td>
		    						<td>${patternSwitch.testLogItem.boxId}</td>
		    						<td>${patternSwitch.testLogItem.callType}</td>
		    						<td>${patternSwitch.timeValue}</td>
		    						<td>${patternSwitch.vmos}</td>
	    							<td>${patternSwitch.keyParameterCause}</td>
		    						<td>${patternSwitch.cellName}</td>
		    						<td>${patternSwitch.beforeSwitchTM}</td>
		    						<td>${patternSwitch.afterSwitchTM}</td>
		    						<td>${patternSwitch.optimization}</td>
		    						<td>${patternSwitch.testLogItem.fileName}</td>
		    						<td>${patternSwitch.testLogItem.recSeqNo}</td>
		    						<td>${patternSwitch.latitude}</td>
		    						<td>${patternSwitch.latitude}</td>
		    						<td>${patternSwitch.latitude}</td>
		    						<td>${patternSwitch.longitude}</td>
		    						<td>${patternSwitch.longitude}</td>
		    						<td>${patternSwitch.longitude}</td>
		    						<td>${patternSwitch.m_stRoadName}</td>
		    					</tr>
	    					</c:forEach>
	    					<c:if test="${null == patternSwitchs}">
	    						<tr>
		    						<td></td>
		    						<td>未选择测试日志</td>
		    						<td></td>
			    ...
			    <div data-options="region:'center',border:false" style="padding:2px 0px 0px 0px;">
  					<table id="patternSwitchKeyIndexTable" class="easyui-datagrid" data-options="title:'关键指标',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
				   		<thead>
				   			<tr>
		    	...
		    	<div data-options="region:'south',border:false" style="height:34%;padding:2px 0px 0px 0px;">
				   	...
				   	<table id="patternSwitchAdviceTable" class="easyui-datagrid" data-options="title:'优化建议：调整模式转换设置值',scrollbarSize:0,fit:true,striped:true,fitColumns:true,remoteSort:false,multiSort:true" >
	    				<thead>
	    					<tr>
