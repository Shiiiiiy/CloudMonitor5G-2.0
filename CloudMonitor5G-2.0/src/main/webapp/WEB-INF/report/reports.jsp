  ...
  <head>
	...
	$(function(){
		...
		function addVideoTable(obj,a){
			...
			var url ;
			url = "${pageContext.request.contextPath}/report/goVideo.action?typeNo="+a+"";
		...
		function addVoiceTable(obj,a){
			...
			obj.style.color = "#f00"; //令当前标签高亮 
			var url ;
			url = "${pageContext.request.contextPath}/report/goVoice.action?typeNo="+a+"";
			$('#cc').layout('panel','center').panel({content:createIframe(url)});
		}	
		/*打开信息报表页*/
		function addDataTable(obj,a){
			...
			obj.style.color = "#f00"; //令当前标签高亮 
			var url ;
			url = "${pageContext.request.contextPath}/report/goData.action?typeNo="+a+"";
			$('#cc').layout('panel','center').panel({content:createIframe(url)});
		}	
		/*打开NB-IoT业务报表页*/
		function addNbiotTable(obj,a){
			var alist = document.getElementsByTagName("a");
			for(var i =0;i < alist.length;i++){
			alist[i].style.color = "#000"; //给所有a标签赋原色
			}
			obj.style.color = "#f00"; //令当前标签高亮 
			var url ;
			url = "${pageContext.request.contextPath}/report/goNbiot.action?typeNo="+a+"";
			$('#cc').layout('panel','center').panel({content:createIframe(url)});
		}	
		
		/* 返回 统计任务详情 */
		function goToReportInfo(){
			goToPage('${pageContext.request.contextPath}/report/seeInfo.action');
		}
	</script>
  </head>
...
<div id="planId" data-options="region:'west',title:' ',split:false,tools:'#tt3'" class="easyui-accordion" style="width:20%;height:100%;">
	...
	<div  title="中国移动NB-IoT" >
		<a id="nbiotButton" class="easyui-linkbutton" style="width:100%;" onclick="addNbiotTable(this,0);"  >NB-IoT业务报表</a>
	</div>
</div>
<div id="tt3">
